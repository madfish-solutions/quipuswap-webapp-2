...
export function Led({ defaultData, getData, dto }: LedDecoratorMetadata): PropertyDecorator {
  return function (target: any, propertyKey: string | symbol): void {
    const pervValues = Reflect.getOwnMetadata<Array<LedMetadataValue>>(LED_METADATA_KEY, target) ?? [];
    Reflect.defineMetadata(LED_METADATA_KEY, [...pervValues, { propertyKey, defaultData, getData, dto }], target);
  };
}
