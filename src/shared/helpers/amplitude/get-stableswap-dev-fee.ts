...
import { Nullable, PartialStableswapDevFee } from '@shared/types';
...
export const getStableswapDevFee = async (tezos: Nullable<TezosToolkit>) => {
  ...
  const { storage } = await getStorageInfo<PartialStableswapDevFee>(tezos, STABLESWAP_FACTORY_CONTRACT_ADDRESS);
