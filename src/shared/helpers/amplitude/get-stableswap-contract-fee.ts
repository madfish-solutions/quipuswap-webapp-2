...
import { Pools, StableswapFactoryContractField, StableswapPoolContractField } from '@shared/types/amplitude';
...
export const getStableswapContractFee = async (tezos: Nullable<TezosToolkit>, stableswapContracAddress: string) => {
  ...
  const { storage: stableswapFactoryStorage } = await getStorageInfo<StableswapFactoryContractField>(
    tezos,
    STABLESWAP_FACTORY_CONTRACT_ADDRESS
  );
  const { storage: stableswapPoolStorage } = await getStorageInfo<StableswapPoolContractField>(
    tezos,
  ...
  const calcualatedFee = Object.values(fee)
    .reduce((prev, curr) => prev.plus(curr), ZERO_AMOUNT_BN)
    .plus(devFee);
