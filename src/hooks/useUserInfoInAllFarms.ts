...
export const useUserInfoInAllFarms = () => {
  ...
  const loadAmountOfTokensInFarms = useCallback(async () => {
    if (!farms) return;
  ...
  }, [farms, accountPkh, farmingStorage, loadAmountOfTokensInFarms]);

  return { userInfoInFarms, loadAmountOfTokensInFarms };
};
