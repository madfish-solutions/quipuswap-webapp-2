...
const RealForm:React.FC<LiquidityFormProps> = ({
  ...
  const rebalanceLiquidityHandler = useCallback(async (val, localTezos, localDex) => {
    ...
    try {
      ...
      const initialBto$ = estimateTezInToken(localDex.storage,
      ...
      const val2 = toDecimals(bal2, token2.metadata.decimals)
        .minus(estimateTokenInTez(localDex.storage, total$));
  ...
  const addLiquidityHandler = useCallback(
    async (isTokensSame, isValuesSame, isDexSame, val, localTezos, localDex) => {
      ...
      const rate = toDecimals(localDex.storage.storage.token_pool, TEZOS_TOKEN.metadata.decimals)
        .dividedBy(
          toDecimals(localDex.storage.storage.tez_pool, token2.metadata.decimals),
        );
      const retValue = lastChange === 'balance1' ? new BigNumber(val.balance1).times(rate) : new BigNumber(val.balance2).div(rate);
  ...
  const handleInputChange = useCallback(async (val: LiquidityFormValues) => {
    ...
    if (val.rebalanceSwitcher !== formValues.rebalanceSwitcher) setAddLiquidityParams([]);
    ...
    const localSaveFunc = async () => {
      ...
      if (values.rebalanceSwitcher !== formValues.rebalanceSwitcher) {
        ...
        if (!values.rebalanceSwitcher) {
          ...
          const retValue = lastChange === 'balance1' ? new BigNumber(values.balance1).times(rate) : new BigNumber(values.balance2).dividedBy(rate);
        ...
        <Field initialValue="0.5 %" name="slippage">
          {({ input }) => {
            const slipPercA = slippageToBignum(values.slippage)
              .times(new BigNumber(values.balanceA ?? 0));
            const slipPercB = slippageToBignum(values.slippage)
              .times(new BigNumber(values.balanceB ?? 0));
            const minimumReceivedA = parseTezDecimals(new BigNumber(values.balanceA ?? 0).minus(slipPercA).toString() ?? '0');
            const minimumReceivedB = parseTezDecimals(new BigNumber(values.balanceB ?? 0).minus(slipPercB).toString() ?? '0');
            ...
            if (dex) {
              ...
              try {
                ...
                const totalB = fromDecimals(estimateTokenInTez(dex.storage, total$),
                  token2.metadata.decimals);
                maxInvestedA = totalA
                  .minus(slippageToBignum(values.slippage).times(totalA));
                maxInvestedB = totalB
            ...
            return (
              <>
                {(currentTab.id === 'remove' || values.rebalanceSwitcher) && (<Slippage handleChange={(value) => input.onChange(value)} />)}
                {currentTab.id === 'add' && values.rebalanceSwitcher && (
                  <>
