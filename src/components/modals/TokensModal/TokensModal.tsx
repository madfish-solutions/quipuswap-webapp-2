...
import {
  useAddCustomToken, useSearchCustomTokens, useSearchTokens, useTokens, useNetwork,
} from '@utils/dapp';
...
export const TokensModal: React.FC<TokensModalProps> = ({
  ...
  React.useEffect(() => handleTokenSearch(), [tokens, inputValue, inputToken, network]);
  const isSoleFa2Token = React.useMemo(
    () => filteredTokens.every((x) => x.type === 'fa2'), [filteredTokens],
  );
  return (
    <Form
