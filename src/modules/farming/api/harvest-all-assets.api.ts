...
export const getHarvestAll = async (tezos: TezosToolkit, farmingIds: BigNumber[], rewardsReceiver: string) => {
...
export const getHarvestAllParams = async (tezos: TezosToolkit, farmingIds: BigNumber[], rewardsReceiver: string) => {
  return (await getHarvestAll(tezos, farmingIds, rewardsReceiver)).map(op => op.toTransferParams());
};

export const harvestAllAssets = async (tezos: TezosToolkit, farmingIds: BigNumber[], rewardsReceiver: string) => {
  const harvestParams = await getHarvestAll(tezos, farmingIds, rewardsReceiver);
