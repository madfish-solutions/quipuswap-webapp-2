...
import { fullRewardIsAvailable } from '@modules/farming/helpers';
...
export const useHarvestAndRollModalViewModel = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const quipuRewards = farmingListStore.rewards.filter(
        reward => isQuipuToken(reward.rewardToken) && fullRewardIsAvailable(reward)
      );
  ...
  }, [opened, getCoinflipGeneralStats, harvestAndRollStore, getUsd, farmingListStore]);
