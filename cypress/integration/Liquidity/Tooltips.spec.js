...
describe('Tooltips', () => {
  it('Should_DisplayTooltips_When_HowerMouseOverTheTooltipIcons', () => {
    ...
    cy.get('[data-test-id="poolDetails"] [data-test-id="sellPrice"] [data-test-id="tooltip"]').trigger('focus');
    cy.get('[aria-describedby="tippy-3"]').should('be.visible');

    cy.get('[data-test-id="poolDetails"] [data-test-id="buyPrice"] [data-test-id="tooltip"]').trigger('focus');
    cy.get('[aria-describedby="tippy-4"]').should('be.visible');

    cy.get('[data-test-id="poolDetails"] [data-test-id="tokenALocked"] [data-test-id="tooltip"]').trigger('focus');
    cy.get('[aria-describedby="tippy-5"]').should('be.visible');

    cy.get('[data-test-id="poolDetails"] [data-test-id="tokenBLocked"] [data-test-id="tooltip"]').trigger('focus');
