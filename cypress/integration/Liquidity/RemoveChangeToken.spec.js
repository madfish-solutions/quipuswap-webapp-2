...
describe('Remove liquidity tab, calculating field', () => {
    ...
    it('Should_HaveKusdQuipuPair_When_TokenIsChanged', () => {
        ...
        cy.get('[data-test-id="liquidityPageTokenSelect"] [data-test-id="positionSelectInput"] [data-test-id="selectLPButton"]')
            .invoke('text').should('contain', 'QUIPU / KUSD');
    });
    it('Should_DisplayMaxInvested_When_InputIs1QuipuKusd', () => {
        ...
        cy.get('[data-test-id="amount"]')
            .invoke('text').then(parseFloat).should('be.gte', 0);
        cy.get('[data-test-id="amount"]').eq(1)
            .invoke('text').then(parseFloat).should('be.gte', 0);
    });
    it('Should_DisplayQuipuKusdInDetailsSection_When_InputIs1', () => {
        //Sell price in pool details section 1 quipu 
        cy.get('[data-test-id="detailsCardCells"] [data-test-id="amount"]').eq(0)
            .invoke('text').then(parseFloat).should('be.eq', 1);
        cy.get('[data-test-id="detailsCardCells"] [data-test-id="rightVisibleCurrency"]').eq(0)
            .invoke('text').should('contain', 'QUIPU');
        //sell price in pool details section =kusd
        cy.get('[data-test-id="detailsCardCells"] [data-test-id="rightVisibleCurrency"]').eq(1)
            .invoke('text').should('contain', 'KUSD');
        //Buy price in pool details section 1kusd = ...quipu
        cy.get('[data-test-id="detailsCardCells"] [data-test-id="amount"]').eq(2)
            .invoke('text').then(parseFloat).should('be.eq', 1);
        cy.get('[data-test-id="detailsCardCells"] [data-test-id="rightVisibleCurrency"]').eq(2)
            .invoke('text').should('contain', 'KUSD');
        cy.get('[data-test-id="detailsCardCells"] [data-test-id="rightVisibleCurrency"]').eq(3)
            .invoke('text').should('contain', 'QUIPU');
    });
    it('Should_DisplayNotification_When_PoolDoesntExist', () => {
        ...
        cy.get('[data-test-id="buttonSelect"]').click();
        cy.get('[data-test-id="liquidityPageTokenSelect"] [data-test-id="connectButton"]').prev()
